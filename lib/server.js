...
var weather = getServiceCreds(appEnv, "weather-service"),
    wService = require("./weatherService").create(weather.url, weather.apiKey);
...
// Route call for api docs
server.route({
    method: 'GET',
    path:'/api-docs/{param*}',
    handler: { directory: { path: "api" } }
})
...
server.route({
  method:  "GET",
  path:    "/api/v1/db/test/shipments/notify",
...
function api_createShipment(request, response) {
  dbHelper.insertRecord(db, request.body, response);
}
...
function api_notifyStoreOwner(request, response) {
  ...
  dbHelper.getRecord(db, 'items', 'shipments_index', shipmentID, function (error, result) {
    if (error) {
      response(error);
      return;
    }

    pushNotifcationsHelper(result, environment, function (data) {
      response(data);
    });
  });
