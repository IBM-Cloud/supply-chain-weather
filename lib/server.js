...
var cloudantCreds = getServiceCreds(appEnv, "supply-chain-datastore"),
    ...
    pushNotificationsHelper = require("./pushNotificationsHelper.js"),
    dbName = "inventory",
    db;
...
//---Set up Weather-------------------------------------------------------------
var weatherInsights = getServiceCreds(appEnv, "supply-chain-weather-insights"),
    wService = require("./weatherService").create(weatherInsights.url, weatherInsights.port);
...
server.route({
  method:  "GET",
  path:    "/api/v1/conditions/currentConditions",
  handler: api_currentConditions
})

// Route API calls to retrieve weather forecast
server.route({
  method:  "GET",
  path:    "/api/v1/conditions/forecastedConditions",
...
function api_notifyStoreOwner(request, response) {
  ...
  dbHelper.getRecord(db, 'shipments', 'shipments_index', shipmentID, function (error, result) {
    ...
    pushNotificationsHelper.notifyShipmentCreated(result, environment, function (data) {
      response(data);
    });
  });
}
...
function api_updateShipmentStatusFromPush(request, response) {
  ...
  async.waterfall([
      ...
      function (result, next) {
        if (status === "shipped") {
          pushNotificationsHelper.notifyShipmentShipped(result, environment, function (data) {
