...
//------------------------------------------------------------------------------
function createService(url, port) {
  return new Service(url, port);
}

//------------------------------------------------------------------------------
function Service(url, port) {
  this.url    = url + ":" + port + "/api/weather";
  this.version = "/v2";
...
function Service__getCurrentConditions(lat, lon, units, cb) {

  // Builds REST URL for requesting current weather
  var url = this.url + this.version + this.observations + "/current";
...
function Service__getForecastedConditions(lat, lon, cb) {

  // Builds REST URL for requesting current weather
  var url = this.url + this.version + this.forecasts + "/10day";
