...
//------------------------------------------------------------------------------
function createService(url, apiKey, insightsUrl, port) {
  return new Service(url, apiKey, insightsUrl, port);
}

//------------------------------------------------------------------------------
function Service(url, apiKey, insightsUrl, port) {
  this.url    = url;
  this.apiKey = apiKey;
...
function Service__getForecastedConditions(lat, lon, cb) {
  ...
  // Builds REST URL for requesting current weather
  var url = this.insightsUrl + this.version + this.forecasts + "/10day";
